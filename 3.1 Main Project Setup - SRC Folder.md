...
├── src/                     # Source code - COMPLETE CODE BASE
...
├── shared/                  # Custom modified packages
├── package.json            # Project dependencies and scripts
└── webpack.*.ts           # Build configurations
```
...
```
Page 1 (index.html) → Click Link → Page 2 (about.html)
    ↓                       ↓
...
```
Single HTML (index.html) → Click Link → Same HTML
    ↓                            ↓
...
In a Single Page Application (SPA), pages don’t reload, so managing data across components can be tricky. State management libraries like Redux provide a central store for app data, allowing components to access and update state consistently, ensuring predictable UI updates without page refreshes. In contrast, multi-page applications (MPAs) reload the page on navigation, and each page fetches data from the server separately, making it harder to share state across pages.
 ...
 * 
 * Without PersistGate:
 * - Page reload → State reset → Data lost
 * - User needs to re-authenticate
 * - Chat history lost
 * 
 * With PersistGate:
 * - Page reload → State restored → Data preserved
...
```
Initial Load:
Redux Store → Persist Config → Save to localStorage
      ↓
   App uses
   fresh state
      ↓
Browser Reload:
localStorage → Persist Config → Rehydrate Store
      ↓
   App uses
 restored state
```
...
```
User clicks link → Router updates URL → Router renders component → Content updates
    ↓                   ↓                     ↓                     ↓
